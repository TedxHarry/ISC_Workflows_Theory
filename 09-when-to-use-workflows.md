...
Access-request governance is its own owner, and it is easy to overlook because it feels like something a workflow should do. Request Center and ISC access-request configuration own the governed request and approval process: what can be requested, which approval path applies, and the record of that decision. A workflow does not replace this. Through Adaptive Approval a workflow can participate in that process, using the Access Request Submitted trigger and an Approval Policy action to make the governed decision, while separate event-driven workflows can react around the process, using Access Request Decision for the outcome and Provisioning Completed for the provisioning stage. Provisioning, in turn, owns fulfillment on the target system. So the clean division is this: native access-request governance owns request and approval, provisioning owns target fulfillment, Manage Access submits a request from within a workflow, and event-driven workflows observe and react around those processes. When you catch yourself rebuilding Request Center governance out of generic forms and manual grants, you are duplicating a control the platform already owns.
