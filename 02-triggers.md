...
Licensed or feature-dependent. Some triggers only appear when the tenant has the matching capability and required setup. CAEP triggers react to shared-signal security events such as a credential change or a session revocation. CIEM triggers react to cloud infrastructure entitlement events. DAS Activity Alert reacts to data access security alerts. Outlier Detected fires when an identity has unusual access compared with its peers. For Outlier specifically, current prerequisites include Access Insights, a configured source with loaded account data, and onboarding that account data into AI-Driven Identity Security. Identity Outliers also has regional availability limits. Reach here for security-driven automation, and when a feature-dependent trigger is missing or does not produce expected events, verify both feature availability and documented prerequisites before blaming the workflow.
