...
Reach for Identity Created whenever the trigger is "a new person now exists." Welcome emails, opening a starter ticket, notifying a manager, kicking off birthright access reviews. One honest caution to hold now and remember later: "created in ISC" means the identity was correlated and built, which is not always the same instant the person walks in the door, and some attributes may still be filling in on that first pass. So this trigger is perfect for "someone new arrived," and less perfect for "every attribute about this person is final and complete." Keep that distinction in your pocket.
...
Notice how close that is to Identity Created and Identity Attributes Changed. The identity family mostly shares this one shape, an identity object plus attributes, which is a good thing to internalize once and carry forward. But the timing of Identity Deleted is the whole point. If you build offboarding on Identity Deleted, you will act too late, because by then the identity and much of its context are already gone. So hold this rule of thumb: use Lifecycle State Changed to drive offboarding when someone leaves, use Lifecycle State Change Processed to react after the automatic cleanup, and treat Identity Deleted as a signal for final housekeeping and audit, not for timely access removal.
