...
Now change only one fact: `score = 0.93`. SailPoint currently provides a high-score template that disables accounts and notifies the manager. A strong answer does not jump from that template to "always disable." It asks what Acme policy authorizes, which identities and accounts are in scope, how exceptions and recovery work, and what evidence will prove the intended containment occurred. If the implementation uses Get Accounts and Manage Accounts, it treats account discovery and action capability as separate gates: an account appearing in Get Accounts does not prove that its source and connector configuration support automatic Disable. Before sending an account to Manage Accounts, verify that the source supports the required Disable/provisioning operation. Accounts on sources where ISC cannot perform that operation follow an alternate manual containment or escalation path instead of being counted as automatically contained. For accounts that pass the capability gate, account for the documented 250-account maximum on Get Accounts, use the intended account ids with Account Action set to Disable, inspect `successfulAccounts`, `failedAccounts`, and `accountsErrorDetails`, and remember the 1-hour Manage Accounts timeout. The destructive step is simulated during logic testing, and target state is verified when the control requires certainty.
...
You can explain what Outlier Detected proves and what it does not, interpret the raw `0.0-1.0` trigger score without confusing it with the Product UI `0-100` display, distinguish the sparse trigger seed from richer UI context, design a policy-driven notification, certification, or containment response, verify source capability before automatic Disable, route unsupported sources to an alternate manual path, debug the score and response boundaries, and make destructive containment safe to test and repeat.
