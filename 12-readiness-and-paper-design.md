...
Now failure and limits. Manage Accounts has an action-specific timeout of 1 hour, so a slow target-system operation has a different ceiling from an HTTP Request or Get Identity. The loop also matters operationally because loop executions count toward the individual workflow's total. SailPoint warns at 200,000 total workflow plus loop executions and blocks remaining executions at 300,000. The tenant-wide daily rate limit is separate: around 400,000 executions, excluding loop executions, after which executions continue at 5 per second for the rest of the day. For a daily contractor-expiration population, the count should normally be small, but you still validate that assumption rather than treating scale as somebody else's problem.
